...
export function generateHistoryPDF(receipts, filtros) {
    return new Promise((resolve, reject) => {
        ...
        const cols = [
            ...
            { label: 'Data',         width: 68,  align: 'left'  },
            { label: 'Beneficiário', width: 162, align: 'left'  },
            { label: 'Banco',        width: 90,  align: 'left'  },
            { label: 'Tipo',         width: 82,  align: 'left'  },
        ...
        // ── Footers — drawn after all content to avoid cursor interference ───
        const range = doc.bufferedPageRange();
        const totalPages = range.count;
        for (let i = 0; i < totalPages; i++) {
            doc.switchToPage(range.start + i);
            // Zerar margem inferior evita que o PDFKit crie página extra ao
            // renderizar texto abaixo do limite de conteúdo da página.
