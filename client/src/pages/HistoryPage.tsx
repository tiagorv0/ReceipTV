...
function buildParams(filters: Filters, sort: string): Record<string, string> {
    return { ...filtersToParams(filters), ...(sort !== 'date_desc' ? { sort } : {}) };
}

function receiptDateStr(r: Receipt): string | null {
    try {
...
const HistoryPage = () => {
    ...
    const [editModal, setEditModal]       = useState<{ open: boolean; receipt: Receipt | null }>({ open: false, receipt: null });
    const [filtersOpen, setFiltersOpen]   = useState(false);
    const [sortBy, setSortBy]             = useState(searchParams.get('sort') ?? 'date_desc');
    const [visibleCount, setVisibleCount] = useState(10);
    const [pendingFilters, setPendingFilters] = useState<Filters>(() => readFiltersFromURL(searchParams));
    const [appliedFilters, setAppliedFilters] = useState<Filters>(() => readFiltersFromURL(searchParams));
    ...
    useEffect(() => {
        setLoading(true);
        getReceipts({ startDate: appliedStart, endDate: appliedEnd })
    ...
    const filteredSorted = useMemo(() => {
        ...
        const filtered = receipts.filter(r => {
            ...
            if (minVal !== null && !isNaN(minVal) && Number(r.valor) < minVal) return false;
            if (maxVal !== null && !isNaN(maxVal) && Number(r.valor) > maxVal) return false;
        ...
        return [...filtered].sort((a, b) => {
            switch (sortBy) {
                case 'date_asc':   return new Date(a.data_pagamento).getTime() - new Date(b.data_pagamento).getTime();
                case 'date_desc':  return new Date(b.data_pagamento).getTime() - new Date(a.data_pagamento).getTime();
                case 'value_asc':  return Number(a.valor) - Number(b.valor);
                case 'value_desc': return Number(b.valor) - Number(a.valor);
    ...
    function removeFilter(key: string): void {
        const updates: Partial<Filters> =
            ...
            key === 'date'  ? { startDate: FIRST_DAY, endDate: LAST_DAY } :
                              { [key]: DEFAULT_FILTERS[key as keyof Filters] };
    ...
    function handleSortChange(valor: string): void {
        setSortBy(valor);
    ...
    function handleNomeChange(value: string): void {
    ...
    // ─── Export ───────────────────────────────────────────────────────────────
    const handleExport = async (formato: 'pdf' | 'zip', delivery: ExportParams['delivery'], emailAddr?: string): Promise<void> => {
        ...
        try {
            ...
            const blob     = new Blob([response.data as Blob], { type: formato === 'pdf' ? 'application/pdf' : 'application/zip' });
            ...
            const axErr = err as { response?: { data?: Blob | { error?: string } } };
            ...
            } else if (axErr.response?.data && typeof axErr.response.data === 'object' && 'error' in axErr.response.data) {
                message = (axErr.response.data as { error?: string }).error || message;
            }
            alert(message);
        } finally {
    ...
    // ─── Edit ─────────────────────────────────────────────────────────────────
    const handleEdit = (receipt: Receipt): void => setEditModal({ open: true, receipt });
    const handleEditClose = (): void => setEditModal({ open: false, receipt: null });
    const handleEditSave = (updated: Receipt): void => {
        setReceipts(prev => prev.map(r => r.id === updated.id ? updated : r));
        setEditModal({ open: false, receipt: null });
    };

    // ─── Delete ───────────────────────────────────────────────────────────────
    const handleDelete = (e: MouseEvent, id: number): void => { e.preventDefault(); setDeleteModal({ open: true, id }); };
    const cancelDelete = (): void => setDeleteModal({ open: false, id: null });
    ...
    // ─── WhatsApp share ───────────────────────────────────────────────────────
    const shareWA = async (r: Receipt): Promise<void> => {
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-green-600/10 p-8 rounded-xl border border-green-500/30 relative overflow-hidden group transition-all hover:bg-green-500/10">
                    ...
                    <h3 className="text-4xl font-headline font-extrabold text-green-500 tracking-tight">
                        {formatCurrency(filteredSorted.reduce((acc, r) => acc + Number(r.valor), 0))}
                    </h3>
                </div>
            </section>
