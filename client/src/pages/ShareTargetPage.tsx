...
const ShareTargetPage = () => {
  ...
  async function processShare(): Promise<void> {
    ...
    if (shareError) {
      navigate('/upload', {
        replace: true,
        state: { shareError: SHARE_ERROR_MESSAGES[shareError as keyof typeof SHARE_ERROR_MESSAGES] ?? 'Erro ao receber o arquivo.' },
      })
      return
    }
    ...
    const { id, file, filename, mimeType } = pending as unknown as { id: string; file: ArrayBuffer; filename: string; mimeType: string }
