...
import { CheckCircle, Smartphone, UploadCloud, Loader2} from 'lucide-react';
...
const UploadPage = () => {
    ...
    useEffect(() => {
        ...
        if (shareError) {
            setError(SHARE_ERROR_MESSAGES[shareError as keyof typeof SHARE_ERROR_MESSAGES] ?? 'Erro ao receber o arquivo compartilhado.');
        }
    ...
    const handleModeChange = (newMode: 'ia' | 'manual'): void => {
        setMode(newMode);
        setError('');
        setResult(null);
    ...
    const handleFiles = async (files: FileList | File[]): Promise<void> => {
        ...
        try {
            ...
            setError(axErr.response?.data?.message || 'Erro ao analisar o comprovante. Tente Novamente. Caso persista, utilize o modo manual.');
        } finally {
            setLoading(false);
            if (fileInputRef.current) fileInputRef.current.value = '';
        }
                ...
                <>
                    ...
                    >
                        <input
                            ...
                            onChange={(e) => { e.preventDefault(); e.stopPropagation(); if (e.target.files) handleFiles(e.target.files); }}
