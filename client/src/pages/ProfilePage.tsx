...
type StatusState = 'idle' | 'loading' | 'error' | 'success';
interface FormStatus { state: StatusState; message: string; }
...
const ProfilePage = () => {
    ...
    const [passwordStatus, setPasswordStatus] = useState<FormStatus>({ state: 'idle', message: '' });

    const [deleteModal, setDeleteModal] = useState(false);
    const [deletePassword, setDeletePassword] = useState('');
    const [deleteStatus, setDeleteStatus] = useState<FormStatus>({ state: 'idle', message: '' });
    ...
    const handlePasswordSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            const msg = axErr.response?.data?.error || 'Erro ao atualizar senha';
    ...
    const handleDeleteAccount = async (): Promise<void> => {
        ...
        try {
            ...
            const axErr = err as { response?: { data?: { error?: string } } };
            const msg = axErr.response?.data?.error || 'Erro ao excluir conta';
